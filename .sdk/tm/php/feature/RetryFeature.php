...
// Opensensemap SDK retry feature
...
class OpensensemapRetryFeature extends OpensensemapBaseFeature
{
    ...
    public function init(OpensensemapContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OpensensemapContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OpensensemapContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OpensensemapContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
