...
class OpensensemapMakeError
{
    public static function call(?OpensensemapContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new OpensensemapContext([], null);
        }
        $op = $ctx->op ?? new OpensensemapOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new OpensensemapResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof OpensensemapError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new OpensensemapError('', $msg, $ctx);
        ...
        if ($err instanceof OpensensemapError) {
