...
boru-driven command-line client **and** interactive REPL for the Opensensemap
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/opensensemap-cli)
make build

# 2. See usage (words, entities, env vars)
./opensensemap-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/opensensemap-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/opensensemap-cli list box
   ```
...
```sh
./opensensemap-cli load 1 box          # scalar shorthand for {id:1}
./opensensemap-cli load '{id:1}' box     # explicit match map
```
...
```sh
./opensensemap-cli update '{id:1,name:"new"}' box
```
...
Run with no arguments to open a REPL (prompt `opensensemap>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `box`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/opensensemap-cli`. |
...
(`./opensensemap-cli <expr>`) and interactively (the REPL), and expressions compose the
