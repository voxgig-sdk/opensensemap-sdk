...
MCP server exposing the Opensensemap SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `opensensemap_list` | `entity`, optional `query` map | First page of records as JSON |
| `opensensemap_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `opensensemap_list` and
`opensensemap_load` tools become available in new sessions.
