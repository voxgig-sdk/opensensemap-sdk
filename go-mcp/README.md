...
[MCP](https://modelcontextprotocol.io) server exposing the Opensensemap SDK as
two agent tools — `opensensemap_list` and `opensensemap_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/opensensemap-mcp)
make build

# 2. Provide credentials via the environment
export OPENSENSEMAP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user opensensemap \
  -- /absolute/path/to/opensensemap-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./opensensemap-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// opensensemap_list: first page of records
{ "entity": "box" }
{ "entity": "box", "query": { } }

// opensensemap_load: one record by id
{ "entity": "box", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/opensensemap-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user opensensemap \
     -- "$PWD"/dist/*/opensensemap-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `opensensemap_list` and `opensensemap_load` tools now appear
   in new sessions. Ask the agent to *"list box using opensensemap"*
   and it calls `opensensemap_list` with `{"entity":"box"}`.
...
```sh
export OPENSENSEMAP_APIKEY=sk_live_xxx            # API key
export OPENSENSEMAP_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `opensensemap_list` tool
...
### Call the `opensensemap_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `opensensemap_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `opensensemap_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPENSENSEMAP_APIKEY` | API key sent with every request. |
| `OPENSENSEMAP_BASE` | Optional override of the API base URL. |
...
```sh
./opensensemap-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"opensensemap_load","arguments":{"entity":"box","query":{"id":1}}}}'
```
