// Command Opensensemap-mcp is an MCP server exposing the Opensensemap
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Opensensemap_list  — list records of an entity
//	Opensensemap_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: OPENSENSEMAP_APIKEY carries the API key and
	// OPENSENSEMAP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OPENSENSEMAP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OPENSENSEMAP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOpensensemapSDK(opts)
