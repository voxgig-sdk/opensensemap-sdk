...
import (
	...
	omni "github.com/voxgig-sdk/opensensemap-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpensensemapError); is {
				out := map[string]any{
					"name":    "OpensensemapError",
...
func omniCtx(arg any, client *sdk.OpensensemapSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpensensemapSDK); is {
			client = live
