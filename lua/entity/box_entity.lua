...
local BoxEntity = {}
BoxEntity.__index = BoxEntity


function BoxEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BoxEntity)
  self._name = "box"
  self._client = client
...
function BoxEntity:get_name()
  return self._name
end


function BoxEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BoxEntity.new(self._client, opts)
end
...
function BoxEntity:mark_deleted()
  self._deleted = true
end


function BoxEntity:deleted()
  return true == self._deleted
end


function BoxEntity:data_set(args)
  if args ~= nil then
...
function BoxEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BoxEntity:match_set(args)
  if args ~= nil then
...
function BoxEntity:stream(action, args, callopts)
...
function BoxEntity:_run_op(ctx, post_done)
  local utility = self._utility
