-- Opensensemap SDK Measurement entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local MeasurementEntity = {}
MeasurementEntity.__index = MeasurementEntity


function MeasurementEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MeasurementEntity)
...
function MeasurementEntity:get_name()
  return self._name
end


function MeasurementEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MeasurementEntity.new(self._client, opts)
end
...
function MeasurementEntity:mark_deleted()
  self._deleted = true
end


function MeasurementEntity:deleted()
  return true == self._deleted
end


function MeasurementEntity:data_set(args)
  if args ~= nil then
...
function MeasurementEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MeasurementEntity:match_set(args)
  if args ~= nil then
...
function MeasurementEntity:match_get()
...
function MeasurementEntity:stream(action, args, callopts)
...
function MeasurementEntity:_run_op(ctx, post_done)
  local utility = self._utility
