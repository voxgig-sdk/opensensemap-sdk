...
function SensorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SensorEntity)
  self._name = "sensor"
  self._client = client
...
function SensorEntity:get_name()
  return self._name
end


function SensorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SensorEntity.new(self._client, opts)
end
...
function SensorEntity:mark_deleted()
  self._deleted = true
end


function SensorEntity:deleted()
  return true == self._deleted
end


function SensorEntity:data_set(args)
  if args ~= nil then
...
function SensorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SensorEntity:match_set(args)
  if args ~= nil then
...
function SensorEntity:match_get()
...
function SensorEntity:stream(action, args, callopts)
...
function SensorEntity:_run_op(ctx, post_done)
  local utility = self._utility
