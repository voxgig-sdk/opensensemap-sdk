-- Opensensemap SDK Statistic entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local StatisticEntity = {}
StatisticEntity.__index = StatisticEntity


function StatisticEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StatisticEntity)
...
function StatisticEntity:get_name()
  return self._name
end


function StatisticEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StatisticEntity.new(self._client, opts)
end
...
function StatisticEntity:mark_deleted()
  self._deleted = true
end


function StatisticEntity:deleted()
  return true == self._deleted
end


function StatisticEntity:data_set(args)
  if args ~= nil then
...
function StatisticEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StatisticEntity:match_set(args)
  if args ~= nil then
...
function StatisticEntity:match_get()
...
function StatisticEntity:stream(action, args, callopts)
...
---@param reqmatch StatisticLoadMatch
---@param ctrl? table
---@return Statistic
---@return string? err
function StatisticEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function StatisticEntity:_run_op(ctx, post_done)
  local utility = self._utility
