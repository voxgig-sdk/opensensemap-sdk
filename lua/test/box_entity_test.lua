...
describe("BoxEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Box(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Box(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Box(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = box_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "box." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set OPENSENSEMAP_TEST_BOX_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local box_ref01_ent = client:Box(nil)
    local box_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local box_ref01_match = {}

    local box_ref01_list_result, err = box_ref01_ent:list(box_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(box_ref01_list_result)
    ...
    -- REMOVE
    local box_ref01_match_rm0 = {
      id = box_ref01_data["id"],
    }
    local _, err = box_ref01_ent:remove(box_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local box_ref01_match_rt0 = {}

    local box_ref01_list_rt0_result, err = box_ref01_ent:list(box_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(box_ref01_list_rt0_result)
...
function box_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/box/BoxTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read box test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("OPENSENSEMAP_TEST_BOX_ENTID")
