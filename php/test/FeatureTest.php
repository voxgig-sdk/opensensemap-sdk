...
class FtCtrl extends OpensensemapControl
{
...
class FtClient
{
    ...
    private OpensensemapUtility $utility;

    public function __construct(OpensensemapUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpensensemapUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpensensemapContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpensensemapContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpensensemapContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpensensemapConfig::make_feature($name);
    ...
    private static function buildUrl(OpensensemapSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpensensemapError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpensensemapResponse($fetched);
        }
    ...
    private function fail(OpensensemapContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpensensemapContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
