...
class SensorEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = OpensensemapSDK::test(null, null);
        $ent = $testsdk->Sensor(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Sensor(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Sensor(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = sensor_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "sensor." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OPENSENSEMAP_TEST_SENSOR_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $sensor_ref01_data = null;
        if (count($sensor_ref01_data_raw) > 0) {
            $sensor_ref01_data = Helpers::to_map($sensor_ref01_data_raw[0][1]);
        }

        // LIST
        $sensor_ref01_ent = $client->Sensor(null);
        $sensor_ref01_match = [
...
function sensor_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/sensor/SensorTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["sensor01", "sensor02", "sensor03", "box01", "box02", "box03"] as $k) {
    ...
    $entid_env_raw = getenv("OPENSENSEMAP_TEST_SENSOR_ENTID");
