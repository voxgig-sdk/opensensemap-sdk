...
class StatisticEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = OpensensemapSDK::test(null, null);
        $ent = $testsdk->Statistic(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = statistic_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "statistic." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OPENSENSEMAP_TEST_STATISTIC_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $statistic_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.statistic")));
        $statistic_ref01_data = null;
        if (count($statistic_ref01_data_raw) > 0) {
            $statistic_ref01_data = Helpers::to_map($statistic_ref01_data_raw[0][1]);
        }
...
function statistic_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/statistic/StatisticTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["statistic01", "statistic02", "statistic03"] as $k) {
    ...
    $entid_env_raw = getenv("OPENSENSEMAP_TEST_STATISTIC_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["OPENSENSEMAP_TEST_STATISTIC_ENTID"]);
