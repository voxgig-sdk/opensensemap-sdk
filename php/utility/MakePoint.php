...
// Opensensemap SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class OpensensemapMakePoint
{
    public static function call(OpensensemapContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof OpensensemapError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = OpensensemapHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = OpensensemapHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
