...
from opensensemap_sdk.opensensemap_types import (
    Statistic,
    StatisticLoadMatch,
)
...
class StatisticEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return StatisticEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Statistic:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Statistic().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
