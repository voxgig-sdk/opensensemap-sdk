...
class TestStatisticDirect:

    def test_should_direct_load_statistic(self):
        setup = _statistic_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-statistic", "live" if setup["live"] else "unit")
        if _skip:
...
def _statistic_direct_setup(mockres):
