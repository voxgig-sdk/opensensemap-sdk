...
class StatisticEntity
  ...
  def make
    opts = @_entopts.dup
    StatisticEntity.new(@_client, opts)
  end
  ...
  # @return [Statistic, Hash] the current Statistic data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Statistic fields)
  def match_get
  ...
  #
  # @param reqmatch [StatisticLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Statistic.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Statistic, Hash] the loaded Statistic; raises OpensensemapError on failure
