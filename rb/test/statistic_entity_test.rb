...
class StatisticEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = statistic_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "statistic." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set OPENSENSEMAP_TEST_STATISTIC_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    statistic_ref01_data = nil
    if statistic_ref01_data_raw.length > 0
...
def statistic_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "statistic", "StatisticTestData.json")
  ...
  entid_env_raw = ENV["OPENSENSEMAP_TEST_STATISTIC_ENTID"]
