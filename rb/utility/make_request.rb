...
module OpensensemapUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpensensemapResponse.new({})
    ...
    if fetch_err
      ...
      response = OpensensemapResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpensensemapResponse.new(fetched)
    else
