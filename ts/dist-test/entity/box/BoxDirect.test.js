...
(0, node_test_1.describe)('BoxDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.OpensensemapSDK({
    ...
    (0, node_test_1.test)('direct-load-box', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-box', setup.live))
            return;
        ...
        if (setup.live) {
            const listResult = await client.direct({
                ...
                params: {
                    ...
                    sensor_id: setup.idmap['sensor01'],
                },
            });
    ...
    (0, node_test_1.test)('direct-list-box', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-box', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["box01", "sensor01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.OPENSENSEMAP_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.OpensensemapSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.OPENSENSEMAP_APIKEY,
        }));
        let idmap = env['OPENSENSEMAP_TEST_BOX_ENTID'];
    ...
    const client = new __1.OpensensemapSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=BoxDirect.test.js.map