...
describe('MeasurementEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'measurement.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['OPENSENSEMAP_TEST_MEASUREMENT_ENTID']
  ...
  idmap = env['OPENSENSEMAP_TEST_MEASUREMENT_ENTID']
