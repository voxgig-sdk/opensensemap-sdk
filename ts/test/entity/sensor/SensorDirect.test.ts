...
describe('SensorDirect', async () => {
  ...
  test('direct-list-sensor', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-sensor', setup.live)) return
    if (skipIfMissingIds(t, setup, ["box01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OPENSENSEMAP_TEST_SENSOR_ENTID']
