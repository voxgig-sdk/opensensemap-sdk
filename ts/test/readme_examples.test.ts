...
//     (\`new OpensensemapSDK(...)\` / \`OpensensemapSDK.test(...)\` become
//     \`OpensensemapSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OpensensemapSDK'
...
// rewrite real client construction (\`new OpensensemapSDK(...)\` and any
// \`OpensensemapSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpensensemapSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpensensemapSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
